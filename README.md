# 🎮 Hey, I'm Praveen Kumar! 👋

![Coder GIF](https://media.giphy.com/media/3ohs7KViF3vWLwl8l6/giphy.gif)

![Profile Views](https://komarev.com/ghpvc/?username=PraveenTechWizard&color=blue&style=flat-square)

---

## 🚀 What I'm Up To Now:

- 🧠 Exploring the world of **Game Development** using **Unity** and **C#**.
- 🛠 Still sharpening my skills in **Python**, **Django**, and **ReactJS**.
- 🎮 Currently building: **2D Platformer Game** and experimenting with Unity physics.
- 🌱 Learning more about **game design**, **animations**, and **performance optimization**.
- 🧩 Balancing my passion between **web development** and **interactive gameplay mechanics**.
- 🏹 On a journey to become a **Game Developer + Full Stack Dev** hybrid.

---

## 💼 Who I Am:

- 👨‍🎓 MCA Graduate from **Anna University, BIT Campus**.
- 💡 Passionate about problem-solving, coding challenges, and building cool stuff.
- ⚡️ I believe technology is a creative superpower. I'm here to master it.

---

## 🛠 Skills & Tech:

<div align="center">
  <img src="https://img.shields.io/badge/-Unity-000000?style=for-the-badge&logo=unity&logoColor=white" alt="Unity">&nbsp;
  <img src="https://img.shields.io/badge/-C%23-239120?style=for-the-badge&logo=c-sharp&logoColor=white" alt="C#">&nbsp;
  <img src="https://img.shields.io/badge/-Python-FFD43B?style=for-the-badge&logo=python&logoColor=blue" alt="Python">&nbsp;
  <img src="https://img.shields.io/badge/-ReactJS-61DAFB?style=for-the-badge&logo=react&logoColor=black" alt="ReactJS">&nbsp;
  <img src="https://img.shields.io/badge/-Django-092E20?style=for-the-badge&logo=django&logoColor=white" alt="Django">&nbsp;
  <img src="https://img.shields.io/badge/-JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black" alt="JavaScript">&nbsp;
  <img src="https://img.shields.io/badge/-HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white" alt="HTML5">&nbsp;
  <img src="https://img.shields.io/badge/-CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white" alt="CSS3">
</div>

---

## 🎯 Ongoing Projects

- 🐍 **Snake Game** (Improved mechanics + UI)
- 🎮 **2D Unity Platformer Game**
- 📦 Web app for daily weather updates with Python
- 🔧 More mini-games in the Unity lab 🧪

---
### 🐍 Watch my contributions snake 🐍

![GitHub Snake dark](https://github.com/PraveenTechWizard/PraveenTechWizard/blob/output/github-snake-dark.svg)
---

## 📈 GitHub Stats:

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=PraveenTechWizard&show_icons=true&theme=tokyonight" alt="GitHub Stats">&nbsp;
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=PraveenTechWizard&theme=tokyonight&hide_border=true" alt="GitHub Streak">
</div>


## 📊 LeetCode Progress

<a href="https://leetcode.com/Lucy000" target="_blank">
  <img src="https://leetcard.jacoblin.cool/Lucy000?theme=dark&font=Fira%20Code&ext=heatmap" alt="LeetCode Stats" />
</a>

---

## 🤝 Connect with Me:

<div align="center">
  <a href="https://www.linkedin.com/in/praveen-kumar-j-1bb832hx" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-Praveen%20Kumar-blue?style=for-the-badge&logo=LinkedIn" alt="LinkedIn">
  </a>&nbsp;
  <a href="https://leetcode.com/Lucy000" target="_blank">
    <img src="https://img.shields.io/badge/LeetCode-Lucy000-orange?style=for-the-badge&logo=leetcode" alt="LeetCode">
  </a>
</div>

---

## 🎉 Fun Fact

> Mixing game dev and web dev is like being a wizard with both magic and tech tools! 🧙‍♂️⚙️

---

